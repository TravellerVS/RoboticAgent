...
bool move_to_point(double dest_x, double dest_y)
{
	...
	if(result){
		...
		double rel_angle = angle_to_dest(behavior_sensorReadings.positionSensor.x, behavior_sensorReadings.positionSensor.y, behavior_sensorReadings.positionSensor.t, dest_x, dest_y);
		if(rotate_to_angle(rel_angle) == true){
			//~ printf(" forward \n");
			if(distance<=DISTANCE_XY_CLOSE*3)
			{
...
bool rotate_to_angle(double targetAngle){
	double tolerance = M_PI/300;
	int speed = SPEED_ROTATE_SLOW;
	double angleDifference = angle_difference(behavior_sensorReadings.positionSensor.t,targetAngle);
	bool result = (angleDifference < tolerance) ? true : false;
	if(result==true){
		movement_stop();
	}
	else
...
#define DISTANCE_BETWEEN_POINTS 			200.0
...
void labyrinth_explore(){
	...
	else if(beh_labyrinth_explore.state == LAB_BEH_FIND_NEXT_UNDEF_FIELD)
	{
		...
		{
			printf("calculate best next destination \n");
	...
	else if(beh_labyrinth_explore.state == LAB_BEH_MAP_FIELD_AND_DECIDE)
	{
		...
		if(rotate_to_angle(angel)==true){
			...
			int neighbour_state = (
										...
										&& behavior_sensorReadings.obstacleSensor.left >= DISTANCE_NEAR_SIDES
										&& behavior_sensorReadings.obstacleSensor.right >= DISTANCE_NEAR_SIDES
...
void behaviors_init(){
	...
	int priority;
	for(priority=0; priority<NUMBER_OF_BEHAVIORS; priority++){
		printf("after reset priority=%d ID=%d \n",priority, behavior_struct.priority_list[priority] );
	}
	reset_labyrinth_explore();
	reset_avoid_colision();
	//~ reset_follow_wall();
